...
main_loop <- function() {
	...
	// Gets a distance to the nearest portal
	// Made it into a separate function cause it's a bit chonky
	...
	// Detect changes in portal placement
		...
		for (local j=pos[1]; j<divy; j++) {
			...
			while (::inbounds) {
				...
				if (error > err_term) {
					...
					if (::inbounds == 1) {
						if (!portal[0] && !portal[1] && !portal[2] && !portal[3])
							color <- [255, 255, 0]
						if (portal[0] && portal[1] && portal[2] && portal[3]) {
							temp <- clone lens
							lens[0] = temp[1]*temp[1]*temp[2]*temp[2]*temp[3]*temp[3];
							lens[1] = temp[0]*temp[0]*temp[2]*temp[2]*temp[3]*temp[3];
							lens[2] = temp[0]*temp[0]*temp[1]*temp[1]*temp[3]*temp[3];
							lens[3] = temp[0]*temp[0]*temp[1]*temp[1]*temp[2]*temp[2];
							temp <- lens[0] + lens[1] + lens[2] + lens[3]
							lens[0] /= temp
							lens[1] /= temp
							lens[2] /= temp
							lens[3] /= temp
							color <- gamma_corr(Cyan, Purple, Yellow, Red, lens)
						}
						if (portal[0] && portal[1] && !portal[2] && !portal[3]) {
							temp <- clone lens
							lens[0] = temp[1]*temp[1];
							lens[1] = temp[0]*temp[0];
							temp <- lens[0] + lens[1]
							lens[0] /= temp
							lens[1] /= temp
							color <- gamma_corr(Cyan, Purple, Yellow, Red, lens)
						...
						if (!portal[0] && !portal[1] && portal[2] && portal[3]) {
							temp <- clone lens
							lens[2] = temp[3]*temp[3];
							lens[3] = temp[2]*temp[2];
							temp <- lens[2] + lens[3]
							lens[2] /= temp
							lens[3] /= temp
							color <- gamma_corr(Cyan, Purple, Yellow, Red, lens)
					...
					if (tp) {
						p1 <- tp - 1
						p2 <- (tp > 2) ? 6 - tp : 2 - tp
						next_point <- ::pos[3] + (next_point - ::pos[3]) * 2.1*((origin[p1] - ::pos[3]).Dot(portal[p1].GetForwardVector()) / (next_point - ::pos[3]).Dot(portal[p1].GetForwardVector()))
						::pos[3] = Matrix_Vector_Mult_3D(local_inv[p1], next_point - origin[p1]);
						::pos[3].y *= -1
						::pos[3].z *= -1
						::pos[3] = Matrix_Vector_Mult_3D(local_matrix[p2], ::pos[3]) + origin[p2];
						tp <- 0;
					}
					else {
						::pos[3] = next_point;
...
::gamma_corr <- function(color1, color2, color3, color4, bias) {
	aver <- [0.0, 0.0, 0.0]
	for (local i=0; i<3; i++)
		aver[i] = abs(pow(pow(color1[i],gamma)*bias[0] + pow(color2[i],gamma)*bias[1] + pow(color3[i],gamma)*bias[2] + pow(color4[i],gamma)*bias[3], 1.0/gamma))
	return aver
}
